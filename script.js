...
//TODO  Auto-check harness (paste in DevTools console after implementing)
...
(function () {
    ...
    const mustThrow = (fn, name) => {
        try { fn; fail(name, 'No error thrown'); } catch { pass(name) };
    };
    ...
    try {
        ...
        setText(t2, old);                   //  revert
    } catch (e) { fail('B suite', e); }

    //  C) only() uniqueness
    try {
        if (typeof only === 'function') pass('C1 only() exists');   else fail('C1 only() exists', 'Missing');
        const toolbar = q('[data-ui=]');
        const status = only('[data-ui="status"], toolbar');
        if (status) pass('C2 only() returns the unique node');
    } catch (e) { fail('C suite', e); }

    //  D) aria sync for toggle button
    try {
        const btn = q('[data-ui="toggle"]');
        const before = btn.getAttribute('aria-pressed');
        btn.click();                        //  simulate
        const after = btn.getAttribute('aria-pressed');
        if (after === 'true' || after === 'false') pass('D1 aria-pressed is boolean string');
        else fail('D1 aria-pressed is boolean string', `got ${after}`);
        //  click back to restore
        btn.click();
        if (btn.getAttribute('aria-pressed') === before) pass('D2 aria-pressed restores');
    } catch (e) { fail('D suite', e); }

    //  E) ensureConnected() after replacement
    try {
        if (typeof ensureConnected !== 'function') throw new Error('ensureConnected missing');
        const grid = q('[data-ui="grid"]');
        const first = q('[data-ui="card"]', grid);
        let cta = q('[data-ui="card.cta"]', first);
        //  simulate replacement
        cta.replaceWith(cta.cloneNode(true));
        //  supply reselection logic
        cta = ensureConnected(cta, () => q('[data-ui="card.cta"]', ));
        setText(cta, 'OK after replace');
        if (cta.isConnected && cta.textContent.incudes('OK after replace')) pass('E1 ensureConnected updates new node');
        else fail('E1 ensureConnected updates new node', 'Did not update');
...
//! Option (recommended): import in the console, then run the checks

//  Paste this as a single block in DevTools:

;(async () => {
    const {
        q, qSafe, only, asHTMLElement, addClassAny,
        setText, setAttr, setStyle
    } = await import('./single.js');

    const toolbar = q('[data-ui="toolbar"]');
    console.assert(!!toolbar, 'q() should find toolbar');

    const missing = qSafe('[data-ui="nope"]');
    console.assert(missing === null, 'qSafe() returns null for missing');

    const status = only('[data-ui="status"]', toolbar);
    console.assert(status && status.textContent, 'only() returns the single status');

    try {
        asHTMLElement(document.createElementNS('http://www.w3.org/2000/svg', 'svg'));
        console.assert(false, 'asHTMLElement should have thrown on SVG');
    } catch { /* expected */ }

    const icon = qSafe('svg[data-ui="icon"]', toolbar);
    if (icon) {
        addClassAny(icon, 'active');
        console.assert(icon.classList.contains('active'), 'addClassAny works for SVG');
    }

    console.log('single.js sanity checks passed');
})();

//  Then in the console you can do:
const { q, qSafe, only, asHTMLElement, addClassAny, setText, setAttr, setStyle } = s;
//  …and run the checks.

//* Two sanity notes
//  • Make sure you’re serving the files over a local server (e.g., VS Code Live Server). Some browsers are picky with file:// and modules.
//  • Keep a single script tag in index.html:
